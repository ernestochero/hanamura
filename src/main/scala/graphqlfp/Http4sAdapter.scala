...
object Http4sAdapter {
  ...
  private def execute[R, Q, M, S](
    ...
    interpreter.execute(query.query,
                        query.operationName,
                        query.variables.map(jsonToVariables).getOrElse(Map()))
  ...
  def makeWebSocketService[R, Q, M, S](interpreter: GraphQL[R, Q, M, S]): HttpRoutes[RIO[R, *]] = {
    ...
    def sendMessage(
      ...
      sendQueue.enqueue1(
        WebSocketFrame.Text(s"""{"id":"$id","type":"data","payload":{"data":$data}}""")
      )
                  ...
                  case Some(query) =>
                    ...
                    (for {
                          ...
                          stream
                            .foreach { item =>
                              sendMessage(sendQueue,
                                          id,
                                          ObjectValue(List(fieldName -> item)).toString)
                            }
                            .fork
                      ...
                      error =>
                        sendQueue.enqueue1(
                          WebSocketFrame.Text(
                            s"""{"type":"complete","id":"$id","payload":"${error.toString}"}"""
                          )
                      )
                    )
                }
